...
const manipulate = () => {
  let dayone = new Date(year, month, 1).getDay(); //on which day of the week does the 1st of the month fall
  let lastdate = new Date(year, month + 1, 0).getDate(); //amount of days in the current month
  let dayend = new Date(year, month, lastdate).getDay(); // On which day of the week does the month end
  let monthlastdate = new Date(year, month, 0).getDate(); //what is the month last date
...
prenexIcons.forEach(icon => {
  icon.addEventListener("click", () => {
    month = icon.id === "calendar_previous" ? month - 1 : month + 1;
